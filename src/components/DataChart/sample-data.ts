...
const hourlyData = Array.from({ length: 200 }, (_, i) => {
  ...
  return {
    ...
    supplyApy: {
      ...
      total: Math.min(0.25, Math.max(0.01, 0.07 + Math.random() * 0.1 + Math.cos(progress * Math.PI * 6) * 0.04)),
    },
  };
});
...
const weeklyData = Array.from({ length: 200 }, (_, i) => {
  ...
  return {
    ...
    supplyApy: {
      base: Math.min(
        0.25,
        Math.max(0.01, 0.06 + Math.random() * 0.1 + Math.sin(progress * Math.PI * 2.7) * 0.03 + marketCrash * 0.05),
      ),
      total: Math.min(
        0.25,
        Math.max(0.01, 0.08 + Math.random() * 0.12 + Math.cos(progress * Math.PI * 3.3) * 0.04 + marketBoom * 0.03),
      ),
    },
  };
});
